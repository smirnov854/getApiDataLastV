...
class Tochki4Tyres extends Tyres
{
    ...
    public function read_from_json() {
        ...
        foreach ($json->tires as $tire) {
            ...
            ;
            $amount = (!empty($tire->rest_sk4) ? preg_replace('/[^0-9]/','',$tire->rest_sk4) : 0) +
                      (!empty($tire->rest_sk3) ? preg_replace('/[^0-9]/','',$tire->rest_sk3) : 0) +
                      (!empty($tire->rest_yamka) ? preg_replace('/[^0-9]/','',$tire->rest_yamka) :0 )+
                      (!empty($tire->rest_mkrs) ? preg_replace('/[^0-9]/','',$tire->rest_mkrs) : 0);
            ...
            $this->database_fields["season"] = $tire->season == "Зимняя" ? 2 : ($tire->season == "Летняя" ? 1 : 3);
            ...
            $this->database_fields["pins"] = (!empty($tire->thorn) ? ($tire->thorn == "Да" ? "on" : "off") : "off");
            $this->database_fields["price_retail"] = $tire->price_mkrs;
            $this->database_fields["runflat"] = $tire->runflat == "Да" ? "on" : "off";
